 ...
 *   2、支持用户自定义样式 PROPS.CLASS-NAME
 *   3、功能上扩展
 *      A：默认展示第几张 PROPS.INIT-SLIDE
 *      B：自动轮播间隔时间 PROPS.AUTO
 *      C：是否显示焦点 PROPS.IS-FOCUS
 *      ...
 *
 * ReactSwipe的BUG
 *   第一次渲染组件的时候，会根据当前CHILD的个数计算出轮播图的样式（例如：宽度等），后期数据有更新，CHILD的个数变了，样式不会被重新的进行计算，此时只能通过重新调取组件完成相关事情
 */
    ...
    render() {
        ...
        return <div className="bannerBox">
            <ReactSwipe className={className}
                ...
                {data.map((item, index) => {
                    let {
                        img = require(`../common/images/default.png`),
                        title = ''
                    } = item;
...
/*
 * 基于REACT实现组件化开发,目的是把一些公用的内容都能进行封装,后期直接调取组件使用即可,真正实现模块化开发 =>尽可能提高组件的重复使用率(通用性)是组件开发的核心目的
 *
 *  1、当前组件中展示的数据是可变还是不可变的，如果是可变的，数据需要调取的时候传递（不能在组件中写死）
 *
 *  2、组件的样式是否需要用户自定义
 *
